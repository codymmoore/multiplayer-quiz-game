...
func TestService_VerifyUser_Success(t *testing.T) {
    mockQuerier := &mockQuerier{
        verifyUserFunc: func(context context.Context, id int32) error {
            return nil
        },
    }
    service := ServiceImpl{
    ...
    response, err := service.VerifyUser(nil, &request)
    if err != nil {
        t.Errorf(`service.VerifyUser(nil, request) error = "%v", expected "<nil>"`, err)
    }
    if response == nil {
        t.Error(`service.VerifyUser(nil, request) response = "<nil>", expected non-nil`)
        return
    }
}

func TestServiceImpl_VerifyUser_QueryFailure(t *testing.T) {
    mockQuerier := &mockQuerier{
        verifyUserFunc: func(context context.Context, id int32) error {
            return errors.New("")
        },
    ...
    response, err := service.VerifyUser(nil, &request)
    if err == nil {
        t.Error(`service.VerifyUser(nil, request) error = "<nil>", expected non-nil`)
    }
    if response != nil {
        t.Errorf(`service.VerifyUser(nil, request) response = "%v", expected "<nil>"`, response)
        return
    }
}
...
type mockQuerier struct {
    ...
    verifyUserFunc func(ctx context.Context, id int32) error
}
...
func (q *mockQuerier) VerifyUser(ctx context.Context, id int32) error {
    return q.verifyUserFunc(ctx, id)
}

func assertUserEqualToDB(t *testing.T, actual *api.User, expected *db.User) {
